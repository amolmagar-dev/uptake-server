...
type SchemaExplorerInput = z.infer<typeof schemaExplorerSchema>;

async function executeSchemaExplorer({ action, connectionId, tableName, schema = "public", searchPattern, limit = 10 }: SchemaExplorerInput): Promise<string> {
  ...
  try {
    ...
    switch (action) {
      case "list_tables": {
        ...
        switch (dbType) {
          ...
          case "sqlite":
            ...
            break;
          default:
            return JSON.stringify({ success: false, error: `Unsupported database type: ${dbType}` });
        }
      ...
      case "get_columns": {
        if (!tableName) {
          return JSON.stringify({ success: false, error: "tableName is required for get_columns action" });
        }
        ...
        switch (dbType) {
          ...
          case "sqlite":
            ...
            return JSON.stringify({
              ...
              columns: pragmaResult.rows.map((col: any) => ({
                name: col.name,
                type: col.type,
        ...
        const result = await executeQuery(connection, query, params as any);
        ...
        if (dbType === "postgresql" || dbType === "postgres") {
          ...
          `;
          const pkResult = await executeQuery(connection, pkQuery, [tableName, schema] as any);
        ...
        return JSON.stringify({
          ...
          connectionId,
          tableName,
          schema: ['mysql', 'mariadb'].includes(dbType) ? null : schema,
      ...
      case "get_sample_data": {
        if (!tableName) {
          return JSON.stringify({ success: false, error: "tableName is required for get_sample_data action" });
        }
      ...
      case "get_table_stats": {
        if (!tableName) {
          return JSON.stringify({ success: false, error: "tableName is required for get_table_stats action" });
        }
      ...
      case "search_tables": {
        if (!searchPattern) {
          return JSON.stringify({ success: false, error: "searchPattern is required for search_tables action" });
        }
      ...
      case "get_relationships": {
        if (!tableName) {
          return JSON.stringify({ success: false, error: "tableName is required for get_relationships action" });
        }
        ...
        return JSON.stringify({
          ...
          foreignKeys: result.rows.map((fk: any) => ({
