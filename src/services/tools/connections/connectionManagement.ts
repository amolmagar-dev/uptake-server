 ...
 * Refactored to use LangChain.js and Prisma repositories
 */
...
/**
 * Connection Management Tool Schema
 */
const connectionManagementSchema = z.object({
  action: z.enum(["list", "get", "create", "update", "delete", "test"]).describe("The action to perform"),
  connectionId: z.string().optional().describe("Connection ID or name (required for get, update, delete, test)"),
  filter: z
    .enum(["all", "mysql", "postgresql", "sqlite"])
    .optional()
    .describe("Filter connections by type (for list action)"),
  data: connectionDataSchema.optional().describe("Connection data (for create and update actions)"),
});

type ConnectionManagementInput = z.infer<typeof connectionManagementSchema>;

/**
 * Execute connection management action
 */
async function executeConnectionManagement({
  action,
  connectionId,
  filter = "all",
  data,
}: ConnectionManagementInput): Promise<string> {
  try {
    switch (action) {
      case "list": {
      ...
      case "get": {
        if (!connectionId) {
          return JSON.stringify({ success: false, error: "connectionId is required for get action" });
        }
      ...
      case "create": {
        ...
        if (!name || !type) {
          return JSON.stringify({ success: false, error: "name and type are required" });
        }

        if (type !== "sqlite" && (!host || !database_name)) {
          return JSON.stringify({
            success: false,
            error: "host and database_name are required for non-SQLite connections",
          });
        }
      ...
      case "update": {
        if (!connectionId) {
          return JSON.stringify({ success: false, error: "connectionId is required for update action" });
        }
        if (!data) {
      ...
      case "delete": {
        if (!connectionId) {
          return JSON.stringify({ success: false, error: "connectionId is required for delete action" });
        }
      ...
      case "test": {
        if (!connectionId) {
          return JSON.stringify({ success: false, error: "connectionId is required for test action" });
        }
...
/**
 * Connection Management Tool
 * LangChain.js tool for managing database connections
 */
